...
const orderSubmit = async () => {
  ...
  if (res.data.insertedId) {
    ...
    setTimeout(() => {
      uni.reLaunch({
        url: `/pagesMember/order/orderList?orderStatus=${orderType.value}`,
      })
      // 提交后清空购物车
      cartStore.clearCart()
    }, 800)
...
<template>
  <scroll-view scroll-y="true" class="viewport">
    <!-- 收货地址 -->
    <!--    <navigator-->
    <!--      v-if="addressList?._id"-->
    <!--      class="shipment"-->
    <!--      hover-class="none"-->
    <!--      :url="`/pagesMember/address/address?from=${order}`"-->
    <!--    >-->
    <!--      <view class="user">{{ addressList?.name }} {{ addressList?.mobile }}</view>-->
    <!--      <view class="address">-->
    <!--        {{ addressList?.fullLoction }}-->
    <!--        {{ addressList?.address }}-->
    <!--      </view>-->
    ...
    <view class="goods">
      <navigator
        v-for="item in cartStore.cartSelectedPro as CartItem[]"
        :key="item?.productId"
        :url="`/pages/goods/goods?id=${item.productId}`"
        class="item"
        hover-class="none"
      >
        <image class="picture" :src="item.cover" />
        <view class="meta">
          <view class="name ellipsis">{{ item.productName }}</view>
          <view class="attrs">{{ item.dec }}</view>
          <view class="prices">
            <!-- 当前价格 -->
    ...
    <view class="settlement">
      <view class="item">
        ...
        <text class="number symbol">{{ cartStore.cartTotal.toFixed(2) }}</text>
      </view>
    </view>
  </scroll-view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <view class="total-pay symbol">
      <text class="number">{{ cartStore.cartTotal.toFixed(2) }}</text>
    </view>
    <view @tap="orderSubmit" class="button" :class="{ disabled: cartStore.cartTotal <= 0 }">
      确认订单
    </view>
  </view>
</template>
